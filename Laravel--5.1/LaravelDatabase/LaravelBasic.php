...
//Laravel 让此类操作变得轻而易举，无论使用原始查找、查询语句构造器或是 Eloquent ORM 都可以适用
...
//注意，有两个键加入了这个配置文件数组内：read 及 write。这两个键内包含了数组，里面也包含了一个单独的键：host。read 及 write 连接的其它数据库设置选项将会合并在主要的 mysql 数组
//所以，如果需要在主要的数组内重写值，只需在 read 及 write 数组内放置设置参数即可。在这个例子中，192.168.1.1将被使用在「读取」连接上，而 192.168.1.2 则被使用在「写入」连接上
//数据库的凭证、前缀、编码设置，以及所有其它的选项都被存放在 mysql 数组内，这两个连接将会共用这些选项。
...
监听查找事件#
//如果你希望能够监控到程序执行的每一条 SQL 语句，则可以使用 listen 方法
//这个方法对于纪录查找跟调试将非常有用。你可以在 服务容器 中注册你的查找侦听器：
<?php
...
数据库事务#
//要想在数据库事务中运行一组操作，则可以在 DB facade 中使用 transaction 方法
//如果在事务的闭包内抛出异常，事务将会被自动还原。如果闭包运行成功，事务将被自动提交
...
使用多数据库连接#
//当你使用了多个连接时，则可以通过 DB facade 的 connection 方法来访问每个连接
//传递给 connection 方法的 name 必须对应至 config/database.php 配置文件中的连接列表的其中一个：

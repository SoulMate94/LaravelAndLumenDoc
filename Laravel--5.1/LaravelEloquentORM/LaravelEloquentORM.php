...
数据表名称#
//我们并没有告诉 Eloquent Flight 模型该使用哪一个数据表。除非数据表明确地指定了其它名称，否则将使用类的「蛇形名称」、复数形式名称来作为数据表的名称
//。因此在此例子中，Eloquent 将会假设 Flight 模型被存储记录在 flights 数据表中。你可以在模型上定义一个 table 属性，用来指定自定义的数据表名称：
<?php
...
访问字段的值#
//假设你有一个 Eloquent 模型的实例，则可以通过相对应的属性来访问模型的字段值
//例如，让我们遍历查找所返回的每个 Flight 实例，并且输出 name 字段的值：
...
//由于每个 Eloquent 模型都可以当作一个 查询构造器，所以你可以在查找中增加规则，然后使用 get 方法来获取结果：
...
集合#
//像是 all 以及 get 之类的可以取回多个结果的 Eloquent 方法，将会返回一个 Illuminate\Database\Eloquent\Collection 实例
//Collection 类提供 多种辅助函数 来处理你的 Eloquent 结果。当然，你也可以简单地像数组一样来遍历集合：
foreach ($flights as $flight) {
    echo $flight->name;
}

分块结果#
//如果你需要处理上千笔 Eloquent 查找结果，则可以使用 chunk 命令
...
$flight->save();
//可以针对符合指定查找的任意数量模型进行更新。在这个例子中，所有 active 并且 destination 为 San Diego 的航班，都将会被标识为延迟：
...
//另外你也可以选择使用 $guarded。$guarded 属性应该包含一个你不想要被批量赋值的属性数组。所有不在数组里面的其它属性都可以被批量赋值
// 因此，$guarded 的功能更像是一个「黑名单」。使用的时候应该只选择 $fillable 或 $guarded 中的其中一个
<?php
...
其它创建的方法#
//还有两种其它方法，你可以用来通过属性批量赋值创建你的模型：firstOrCreate 和 firstOrNew。
//firstOrCreate 方法将会使用指定的字段／值对，来尝试寻找数据库中的记录。如果在数据库中找不到模型，则会使用指定的属性来添加一条记录
//firstOrNew 方法类似 firstOrCreate 方法，它会尝试使用指定的属性在数据库中寻找符合的纪录。如果模型未被找到，将会返回一个新的模型实例。
...
//如果模型有一个非空值 deleted_at，代表模型已经被软删除了。要在模型上启动软删除，则必须在模型上使用 Illuminate\Database\Eloquent\SoftDeletes trait 并添加 deleted_at 字段到你的 $dates 属性上：
<?php
...
//而且，当查找有启用软删除的模型时，被软删除的模型将会自动从所有查找结果中排除
...
Where 子句注意事项#
//当在你的软删除模型查找增加了 orWhere 子句时，请使用 高级 where 子句 来对 WHERE 子句的逻辑进行分组
...
查找范围#
//全局作用域（Global Scopes）可让你定义有限制的共用集合，它可以轻松地在你的应用程序中被重复使用
//例如，你可能需要频繁地获取所有被认为是「受欢迎的」用户。要定义此范围，则可以简单地在 Eloquent 模型方法前面加上前缀 scope：范围应总是返回查询构造器的实例：
<?php
...
//让我们在 服务提供者 中定义一个 Eloquent 事件侦听器来作为示例。在我们的事件侦听器中，我们会在指定的模型上调用 isValid 方法，并在模型无效时返回 false。从 Eloquent 事件侦听器中返回 false 的话会取消 save 和 update 的操作：
<?php

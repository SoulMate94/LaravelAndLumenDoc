...
定义关联#
//你可在 Eloquent 模型类内将 Eloquent 关联定义为函数
//因为关联像 Eloquent 模型一样也可以作为强大的 查询语句构造器，定义关联为函数提供了强而有力的链式调用及查找功能。例如
...
一对多#
//一个「一对多」关联使用于定义单个模型拥有任意数量的其它关联模型
//例如，一篇博客文章可能会有无限多个评论。就像其它的 Eloquent 关联一样，可以通过放置一个函数到 Eloquent 模型上来定义一对多关联：
<?php
...
定义相对的关联#
//要定义相对于多对多的关联，只需简单的放置另一个名为 belongsToMany 的方法到你关联的模型上
//让我们接着以用户身份为例，在 Role 模型中定义 users 方法：
<?php
...
获取中间表字段#
//要操作多对多关联需要一个中间数据表。Eloquent 提供了一些有用的方法来和这张表进行交互
//例如，假设 User 对象关联到很多的 Role 对象。访问这些关联对象时，我们可以在模型中使用 pivot 属性来访问中间数据表的数据
...
远层一对多#
//「远层一对多」提供了方便简短的方法来通过中间的关联获取远层的关联
//例如，一个 Country 模型可能通过中间的 Users 模型关联到多个 Posts 模型。让我们来看看定义此种关联的数据表：
countries
    id - integer
    name - string
...
多态关联#
数据表结构#
//多态关联允许一个模型在单个关联中从属一个以上其它模型
//比方说你可为你的工作人员和产品保存一些照片。使用多态关联，你可以对这两种情况使用单个 photos 数据表。让我们先来查看下创建这种关联所需的数据表结构
staff
    id - integer
    name - string
...
多态多对多关联#
//一旦你的数据表及模型被定义，则可以通过模型来访问关联
//例如，若要访问工作人员的所有照片，则可以简单的使用 photos 动态属性：
...
多态多对多关联#
数据表结构#
//除了一般的多态关联，你也可以定义「多对多」的多态关联
//例如，博客的 Post 和 Video 模型可以共用多态关联至 Tag 模型。使用多对多的多态关联能够让你的博客文章及图片共用独立标签的单个列表。让我们先来查看数据表结构：
posts
    id - integer
    name - string
...
获取关联#
//一旦你的数据表及模型被定义，则可以通过你的模型来访问关联
//例如，你可以简单的使用 tags 动态属性来访问文章的所有标签：
...
//你也可以从多态模型的多态关联中，通过访问运行调用 morphedByMany 的方法名称来获取拥有者
//在此例子中，就是 Tag 模型的 posts 或 videos 方法。因此，你可以通过访问使用动态属性来访问这个方法：
$tag = App\Tag::find(1);
...
//如我们刚刚的 User 及 Post 模型示例，我们可以像这样来访问所有用户的文章
$user = App\User::find(1);
...
//正因为如此，开发者通常需要使用 预加载 来预先加载关联数据，关联数据将会在模型加载后被访问。预加载能有效减少你的 SQL 查询语句

查找关联是否存在#
//当访问模型的记录时，你可能希望根据关联的存在来对结果进行限制
...
//意味着该关联数据只有在你使用属性访问它时才会被加载。不过，Eloquent 可以在你查找上层模型时「预加载」关联数据
//预加载避免了 N + 1 查找的问题。要说明 N + 1 查找的问题，可试想一个关联到 Author 的 Book 模型，如下所示
<?php
...
//例如，让我们假设一个用户可以拥有多个身份，且每个身份都可以被多个用户拥有。要附加一个规则至一个用户，并连接模型以及将记录写入至中间表，则可以使用 attach 方法：
$user = App\User::find(1);
...
//有时我们也需要来移除用户的身份。要移除一个多对多的记录，可使用 detach 方法
//detach 方法会从中间表中移除正确的记录；当然，$user 和 $role 这两个模型依然会存在于数据库中：
// 从用户身上移除单个身份...
...
方便的同步#
//你也可以使用 sync 方法来构建多对多关联。
//sync 允许传入放置于中间表的 ID 数组。任何不在指定数组中的 ID 都将会从中间表中删除。所以，在此操作结束后，中间表中只存在数组中的 ID：
...
连动上层时间戳#
//$user->roles()->sync([1 => ['expires' => true], 2, 3]);
//这对于下层模型被更新时，要更新上层的时间戳相当有帮助。举例来说，当一个 Commnet 模型被更新时，你可能想要「连动」更新 Post 所属的 updated_at 时间戳

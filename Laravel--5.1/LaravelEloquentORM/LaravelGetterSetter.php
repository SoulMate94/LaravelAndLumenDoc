...
访问器和修改器#
定义一个访问器#
//若要定义一个访问器，则必须在你的模型上创建一个 getFooAttribute 方法
//要访问的 Foo 字段需使用「驼峰式」来命名。在这个例子中，我们将为 first_name 属性定义一个访问器。当 Eloquent 尝试获取 first_name 的值时，将会自动调用此访问器：
<?php
...
定义一个修改器#
// 若要定义一个修改器，则必须在模型上定义一个 setFooAttribute 方法。
//要访问的 Foo 字段需使用「驼峰式」来命名。让我们再来定义 first_name 属性的修改器。当我们尝试在模型上设置 first_name 的值时，将会自动调用此修改器：
<?php
...
//修改器会获取属性已经被设置的值，让你可以操作该值并将其设置到 Eloquent 模型内部的 $attributes 属性上
...
//例如，在你的数据库中有一个 TEXT 字段类型，其包含了 被序列化的 JSON，且对该属性添加了 array 类型转换。当你在 Eloquent 模型上访问该属性时，它将会被自动反序列化成一个 PHP 数组：
<?php

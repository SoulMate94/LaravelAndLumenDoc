...
#应用程序结构#
#简介
//Laravel 几乎没有对任何类的放置位置进行强加限制 - 只要 Composer 可以自动加载这些类即可。
...
--storage 目录包含编译后的 Blade 模板、基于文件的 session、文件缓存和其它框架生成的文件
	--app、framework，及 logs 目录
	--app 目录可用于存储应用程序使用的任何文件
...
#App 目录
//应用程序的「内容」存在于 app 目录中
//默认情况下，这个目录在 App 命名空间下借助 Composer 使用 PSR-4 自动加载标准自动加载
//你可以使用 app:name Artisan 命令修改这个命名空间。
...
#为应用程序设置命名空间
//默认的应用程序命名空间为 App；然而，你可以简单地借由 app:name Artisan 命令，将此命名空间修改成应用程序的名称
//例如：如果你的应用程序叫做「SocialNet」，你可以运行下方的命令来进行更改：

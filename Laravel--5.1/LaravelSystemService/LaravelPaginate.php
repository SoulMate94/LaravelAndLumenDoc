...
简介#
在其它的框架中，分页是非常让人苦恼的。而在 Laravel 中却是很轻而易举的
// Laravel 可以快速生成基于当前页面链接的智能「请求范围」，并且生成兼容 Bootstrap CSS 框架 的 HTML

基本使用#
对查询语句构造器进行分页#
//有几种方法可以对项目进行分页。最简单的是使用 paginate 方法
//默认状况下，当前页数由 HTTP 请求所带的 ?page 参数来决定。当然，该值由 Laravel 自动检测，并自动插入由分页器生成的链接
...
将分页结果显示在视图中#
当使用 paginate 方法时，将得到 Illuminate\Pagination\LengthAwarePaginator 实例。
当使用 simplePaginate 方法时，则会得到 Illuminate\Pagination\Paginator 实例
...
自定义分页器的 URI#
setPath 方法允许你在生成链接时自定义 URI 
//例如，如果你希望分页器生成像 http://example.com/custom/url?page=N 这样的链接，则应该使用 setPath 方法将 custom/url 加到分页中：
...
//如果你想加入一个有「哈希片段」的分页器链接网址，则可以使用 fragment 方法
//例如，要在每个分页链接的最后加入 #foo ，应该像这样使用 fragment 方法：

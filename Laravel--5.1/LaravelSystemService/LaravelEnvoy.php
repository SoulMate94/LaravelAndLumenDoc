...
简介#
//Laravel Envoy 使用了 Blade 风格的语法，让你可以很方便的进行部署任务设置、Artisan 命令运行等操作。目前，Envoy 只支持 Mac 及 Linux 操作系统。
...
多个服务器#
//首先，增加额外的服务器至你的 @servers 声明，每个服务器必须分配一个唯一的名称。一旦你定义好其它服务器，就能够在任务声明的 on 数组中列出这些服务器：
...
任务宏#
//宏可以让你使用一个命令来定义要顺序运行的一组任务
举例来说，一个 deploy 宏可能会运行 git 及 composer 任务：
@servers(['web' => '192.168.1.1'])
...
运行任务#
//要从你的 Envoy.blade.php 文件运行一个任务，只需运行 Envoy 的 run 命令，并传递你想运行的任务或宏命令的名称
//Envoy 会运行该任务并显示任务运行时的服务器输出。
envoy run task

通知#
HipChat#
//在任务运行之后，你可以使用 Envoy 的 @hipchat 发送通知到团队的 HipChat 聊天室
//该命令接受的参数是 API token、聊天室名称、及发送消息时显示的用户名
@servers(['web' => '192.168.1.1'])
...
Slack#
//除了 HipChat 之外，Envoy 也支持发送通知至 Slack。@slack 命令接收 Slack hook 网址、频道名称、及你想发送至频道的消息：
@after
    @slack('hook', 'channel', 'message')
@endafter

//当你在 Slack 的网站创建 Incoming WebHooks 时会获取一组 webhook 网址
//hook 参数必须是 Slack 的 Incoming WebHooks 所提供的整串网址。例如：

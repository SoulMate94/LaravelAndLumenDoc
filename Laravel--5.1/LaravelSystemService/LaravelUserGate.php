...
#用户授权#
//除了内置提供的 用户认证 服务外，Laravel 还提供了用户授权和资源访问控制的方案

#定义权限
//判断一个用户是否允许执行特定行为，最简单的方式就是使用 Illuminate\Auth\Access\Gate 类定义「权限」
//可以在 AuthServiceProvider 文件中定义应用程序的所有权限
//举个例子，我们需要定义一个 update-post 的权限，需要判断目前的 User 及 Post 模型 是否有所属关系，也就是「文章」是不是「用户」发的，我们会判断用户的 id 与文章的 user_id 是否一致：
<?php
...
#通过用户模型
//，你也可以通过 User 模型的实例检查权限。默认情况下，Laravel 的 App\User 模型使用了 Authorizable trait，它提供了两个方法：can 及 cannot。
...
//首先，让我们生成一个授权策略来管理 Post 模型的授权
//你可以通过 make:policy artisan 命令 生成一个授权策略。生成的授权策略会被放置于 app/Policies 目录中：
php artisan make:policy PostPolicy


#创建授权策略
//一旦该授权策略存在，我们需要将它与 Gate 类进行注册
//AuthServiceProvider 包含了一个 policies 属性，可将各种模型对应至管理它们的授权策略。所以，我们需要指定 Post 模型的授权策略是 PostPilicy 类：
<?php
...
//例如，让我们在 PostPolicy 中定义一个 update 方法，它会判断指定的 User 是否可以「更新」一条 Post
<?php
...
//拦截所有检查
//有时，你可能希望在授权策略赋予所有权限给指定用户。对于这种情况，只要在授权策略中定义一个 before 方法。
//授权策略的此方法会在其它所有授权检查前被运行：
...
//通过授权策略辅助函数
//全局的 policy 辅助函数可以被用于为指定的类实例获取 Policy 类
if (policy($post)->update($user, $post)) {
    //
}

#控制器授权
//默认的，App\Http\Controllers\Controller 类包含了 Laravel 使用的 AuthorizesRequests trait
...
//如果该行为被授权了，控制器将会继续正常运行；但是，如果 authorize 方法判断没有权限运行该行为，那么将会自动生成一个带有 403 Not Authorized 状态码的 HTTP 响应并抛出异常
//如你所见，authorize 方法是进行授权行为或处理抛出异常的一个简单、快速的方法，仅使用了一行程序代码。
//AuthorizesRequests trait 也提供了 authorizeForUser 方法来为当前非认证用户授权行为：
$this->authorizeForUser($user, 'update', $post);

//自动判断授权策略方法
//。在本例中，因为 authorize 被控制器的 update 方法调用，所以也会调用 PostPolicy 中的 update 方法：
/**
 * 更新指定的文章。
 *
 * @param  int  $id

...
简介#
//Laravel 的队列服务为不同的队列后端系统提供了一套统一的 API 。队列允许你将一个耗时的任务进行延迟处理
配置#
队列的配置文件被保存在 config/queue.php 中
//在这个文件内你可以找到包含在 Laravel 中的每一种队列驱动连接设置。它们包含了数据库、Beanstalkd、IronMQ、Amazon SQS、Redis 以及提供本机使用的 synchronous 驱动
...
//注意，在这个例子中，我们在任务类的构造器中直接传递了一个 Eloquent 模型。因为我们在任务类里引用了 SerializesModels 这个 trait，使得 Eloquent 模型在处理任务时可以被优雅地序列化和反序列化
//在队列处理任务时，会调用 handle 方法，而这里我们也可以通过 handle 方法的参数类型提示，让 Laravel 的 服务容器 自动注入依赖对象。


当发生错误的时候#
//如果在处理任务时抛出了一个异常，它将会自动被释放回队列里并再次尝试运行。当该任务一直出错时，它会不断被发布重试，直到超过应用程序所允许的最大重试值
最大重试值可以在运行 queue:listen 或 queue:work 命令时，用 --tries 选项来设置

手动释放任务#
//生成出来的任务类引用了 InteractsWithQueue 这个 trait，它提供了 release 方法让我们可以手动释放任务
//在 release 方法中可指定一个参数，表示此任务被重新运行之前所等待的秒数。
public function handle(Mailer $mailer)
{
    if (condition) {
        $this->release(10);
...
检查重试次数#
//当任务被处理时若发生一些异常，则任务将会被自动释放回队列中。这时候可以用 attempts 方法来检查重试次数：
public function handle(Mailer $mailer)
{
    if ($this->attempts() > 3) {
        //
...
指定任务所属的队列#
//要推送任务到不同的队列上，你需要将任务先「分类」，甚至可能要指定每个队列能有多少作业器可以运行任务
//要指定任务运行的队列，可以用任务实例的 onQueue 方法。onQueue 是 Illuminate\Bus\Queueable trait 所提供的方法，而它已经包含在 App\Jobs\Job 基类中：
<?php
...
//这个方法会检查任务类的构造器，并且从 HTTP 请求 (或任何 ArrayAccess 对象) 中取出变量，来作为构造器中需要的参数
//所以如果我们的任务类构造器接受一个 productId 变量的话，那么队列就会试着从 HTTP 请求中提取 productId 这个参数。
...
$this->dispatchFrom('App\Jobs\ProcessOrder', $request, [
	...
	任务事件#
任务完成事件#
//Queue::after 方法可以让你注册一个队列任务运行完成后会被执行的回调
//在此回调进行额外纪录、队列后续任务、或为仪表板增加统计等操作都是很好的时机
<?php
...
//Supervisor 的配置文件一般是放在 /etc/supervisor/conf.d 目录下，在这个目录中你可以创建任意数量的配置文件来要求 Supervisor 监控你的进程
...
在后台任务侦听器进行布署#
//由于后台队列工作器是长驻进程，因此除非队列重新启动，否则任何代码上的任何修改都不会被应用
//所以要布署一个有用到后台队列服务器的应用程序，最简单的方式就是在布署脚本文件中对作业器进行重启。你可以在布署命令中加入以下命令，来优雅地重启所有作业器：
...
处理失败的任务#
//当任务运行超过该重试次数时，它就会被写入至 failed_jobs 这个数据表中。而失败任务的名称可以在 config/queue.php 这个配置文件中设置。
...
//queue:failed 命令会列出所有任务的 ID、连接、队列以及失败时间，任务 ID 可以被用在重试失败的任务上
//例如要重试一个 ID 为 5 的失败任务，其命令如下：

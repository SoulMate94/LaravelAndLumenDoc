...
简介#
//事实上， Laravel 默认就支持用 PHPUnit 来做测试，并为你的应用程序创建好了 phpunit.xml 文件。框架还提供了一些便利的辅助函数，让你可以更直观的测试应用程序。
...
//此命令会放置一个新的 UserTest 类至你的 tests 目录
//接着就可以像平常使用 PHPUnit 一样来定义测试方法。要运行测试只需要在命令行上运行 phpunit 命令即可：
<?php
...
测试应用程序#
//Laravel 为 HTTP 请求的生成和发送操作、输出的检查，甚至表单的填写都提供了非常流利的 API
//举例来说，你可以看看 tests 目录中的 ExampleTest.php 文件：
<?php
...
//visit 方法会创建一个 GET 请求，
//see 方法则断言在返回的响应中会有指定的文本，
//dontSee 方法断言在返回的响应中不会有指定的文本。这是 Laravel 所提供的最基本的应用程序测试

与你的应用程序进行交互#
//当然，除了断言文本是否存在于一个指定的响应中，你还可以做更多的交互。让我们来看看点击链接及填写表单的例子：

点击链接#
//在这个测试中，我们会生成一个请求并「点击」返回响应中的链接，接着断言我们会停留在指定的 URI 上
//举个例子，假设在响应中有个链接，并写着「About Us」：
...
使用表单#
//Laravel 还提供了几种用于测试表单的方法。通过 type、select、check、attach 及 press 方法让你与表单的所有输入框进行交互
//举个例子，让我们想像一下有个在应用程序注册页面的表单：
...
测试 JSON APIs#
//Laravel 也提供了几个辅助函数来测试 JSON API 及其响应。举例来说，get、post、put、patch 及 delete 方法可以用于发出各种 HTTP 动作的请求
//你也可以轻松的传入数据或标头到这些方法上。首先，让我们来编写一个测试，将 POST 请求发送至 /user ，并断言其会返回 JSON 格式的指定数组
<?php
...
//seeJson 方法会将传入的数组转换成 JSON，并验证该 JSON 片段是否存在于应用程序返回的 JSON 响应中的 任何位置
//也就是说，即使有其它的属性存在于该 JSON 响应中，但是只要指定的片段存在，此测试便会通过
...
//然，一般使用 Session 时都是用于维持用户的状态，如认证用户。actingAs 辅助函数提供了简单的方式来让指定的用户认证为当前的用户。
//举个例子，我们可以使用 模型工厂 来生成并认证用户：
<?php
...
停用中间件#
//测试应用程序时，你会发现，在某些测试中停用 中间件 是很方便的
//这让你可以隔离任何中间件的所有影响，以便更好的测试路由及控制器。Laravel 包含了一个简洁的 WithoutMiddleware trait，你能在测试类中使用它来自动停用所有的中间件：
<?php
...
使用数据库#
//Laravel 也提供了多种有用的工具来让你更容易的测试使用数据库的应用程序
//首先，你可以使用 seeInDatabase 辅助函数，来断言数据库中是否存在与指定条件互相匹配的数据
...
//aravel 提供了简洁的 DatabaseMigrations trait，它会自动帮你处理好这些操作。你只需在测试类中使用此 trait 即可：
<?php
...
//创建测试数据时，除了手动的来设置每个字段的值，还可以使用 Eloquent 模型 的「工厂」来设置每个属性的默认值。
//在开始之前，你可以先查看下应用程序的 database/factories/ModelFactory.php 文件。此文件包含一个现成的工厂定义
...
//闭包内为工厂的定义，你可以返回模型中所有属性的默认测试值
//在该闭包内会接收到 Faker PHP 函数库的实例，它可以让你很方便的生成各种随机数据以进行测试。
//当然，你也可以随意将自己额外的工厂增加至 ModelFactory.php 文件


多个工厂类型#
//例如，除了一般用户的工厂之外，还有「管理员」工厂。你可以使用 defineAs 方法来定义这个工厂：
...
在测试中使用工厂#
//在工厂定义后，就可以在测试或是数据库的填充文件中，通过全局的 factory 函数来生成模型实例
//接着让我们先来看看几个创建模型的例子。首先我们会使用 make 方法创建模型，但不将它们保存至数据库：
public function testDatabase()
{
...
增加关联至模型#
//你甚至可以保存多个模型到数据库上。在本例中，我们还会增加关联至我们所创建的模型
//当使用 create 方法创建多个模型时，它会返回一个 Eloquent 集合实例，让你能够使用集合所提供的便利函数，像是 each：
...
模拟#
模拟事件#
//如果你正在频繁地使用 Laravel 的事件系统，你可能希望在测试时停止或是模拟某些事件
//举例来说，如果你正在测试用户注册功能，你可能不希望所有 UserRegistered 事件的处理进程都被运行，因为它们会触发「欢迎」邮件的发送。
...
模拟任务#
//有时你可能希望当请求发送至应用程序时，简单的对控制器所派送的任务进行测试
//这么做能够让你隔离测试路由或控制器，设置除了任务以外的逻辑。当然，在此之后你也可以在一个单独的测试案例中来测试该任务
...
//我们可以通过 shouldReceive 方法模拟调用 Cache facade，它会返回一个 Mockery 模拟的实例
//因为 facades 实际上已经被 Laravel 的 服务容器 解决并管理着，它们比起一般的静态类更有可测性
举个例子，让我们来模拟调用 Cache facade：
<?php

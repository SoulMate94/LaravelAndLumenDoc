...
你的调度已经定义在 app/Console/Kernel.php 文件的 schedule 方法中
//为了方便你开始，在该方法内包含了一个简单的例子。你可以随意增加调度到 Schedule 对象中。
...
//在该例子中，我们计划了一个会在午夜被调用的闭包。该闭包将运行清除某个数据表的数据库查找：
<?php
...
//换句话说，如果指定的 闭包 返回 true，且没有其它限制条件存在，那么这个任务将会被继续运行
...
//在这个例子中，如果没有其它 emails:send Artisan 命令 在运行的话，此任务将于每分钟被运行一次
//当你有些任务运行时间过长，且无法预测出具体所需时间时，withoutOverlapping 方法将会特别有帮助

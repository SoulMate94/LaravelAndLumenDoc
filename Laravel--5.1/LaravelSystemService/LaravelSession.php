...
Session#
简介#
//由于 HTTP 协议是无状态的，所以 session 提供了一种保存用户数据的方法
//Laravel 附带支持了多种 session 后端驱动，并通过统一的 API 进行使用。也内置支持像是 Memcached、Redis 和数据库这样的后端驱动


配置#
Session 的配置文件在 config/session.php
//请务必看一下此配置文件中可用的设置选项及注释。Laravel 默认使用 file 的 session 驱动，它在大多数应用中可以良好运作
//在上线的应用程序中，你可能会考虑使用更快的 memcached 或 redis 等驱动。
...
闪存数据#
//有时候你想存入一条缓存的数据，让它只在下一次的请求内有效，则可以使用 flash 方法
//使用这个方法保存 session，只能将数据保留到下个 HTTP 请求，然后就会被自动删除。闪存数据在短期的状态消息中很有用：
$request->session()->flash('status', 'Task was successful!');

//如果需要保留闪存数据给更多请求，可以使用 reflash 方法
//这将会将所有的闪存数据保留给额外的请求。如果想保留特定的闪存数据，则可以使用 keep 方法：
$request->session()->reflash();

$request->session()->keep(['username', 'email']);

增加 Session 驱动#
//若要加入额外驱动至 Laravel 的 session 中，则可以使用 Session facade 的 extend 方法。在 服务提供者 的 boot 方法内调用 extend 方法：
<?php

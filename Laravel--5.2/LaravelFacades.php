...
//getFacadeAccessor 方法的工作定义是从容器中解析出什么。
//Facade 基类利用 __callStatic() 魔术方法从你的 facade 延迟调用来解析对象
...
//记住，这个方法的工作是返回服务容器绑定的名称。当用户在 Cache facade 上参考任何的静态方法，Laravel 会从 服务容器 解析被绑定的 cache 以及针对对象运行请求的方法（在这个例子中是 get）


Facade 类参考#
//在下方你可以找到每个 facade 及其底层的类。这个工具对于通过指定 facade 的来源快速寻找 API 文档相当有用。可应用的 服务容器绑定 关键字也包含在里面。

//https://d.laravel-china.org/docs/5.2/facades

...
配置共享文件夹#
//你可以在 Homestead.yaml 文件的 folders 属性里列出所有想与 Homestead 环境共享的文件夹
//这些文件夹中的文件若有变更，它们将会在你的本机电脑与 Homestead 环境自动更新同步。你可以在这里设置多个共享文件夹：
folders:
    - map: ~/Code
...
配置 Nginx 网站#
//对 Nginx 不熟悉吗？没关系。sites 属性可以帮助你可以轻易指定一个 域名 来对应到 homestead 环境中的一个目录上
//在 Homestead.yaml 文件中已包含了一个网站设置范本。同样的，你也可以增加多个网站到你的 Homestead 环境中。Homestead 可以同时为多个 Laravel 应用提供虚拟化环境：
sites:
    - map: homestead.app
      to: /home/vagrant/Code/Laravel/public

//你可以将 hhvm 属性设置为 true 来让 Homestead 里面的任意一个网站改用 HHVM：
sites:
    - map: homestead.app
      to: /home/vagrant/Code/Laravel/public
      hhvm: true
...
//hosts 文件会将请求重定向至 Homestead 环境中设置的本地域名。在 Mac 或 Linux 上，该文件通常会存放在 /etc/hosts。在 Windows 上，则存放于 C:\Windows\System32\drivers\etc\hosts。设置内容如下所示：
...
启动 Vagrant box#
//编辑完 Homestead.yaml 后，在命令行里进入你的 Homestead 目录并运行 vagrant up 命令
vagrant up
//Vagrant 就会根据 Homestead.yaml 里的配置信息，为虚拟机设置共享文件夹和 Nginx 网站
...
安装 MariaDB#
//如果你希望使用 MariaDB 来替换 MySQL，你可以在 Homestead.yaml 文件中增加一个 mariadb 的选项，这个选项会移除 MySQL 并安装 MariaDB
//因为 MariaDB 可用作 MySQL 的替代品，所以在你的数据库配置信息里，还是选用 mysql 配置项。
...
//如果想要从本机电脑上通过 Navicat 或者是 Sequel Pro 来连接数据库，
//可以通过 127.0.0.1 来使用端口 33060 (MySQL) 或 54320 (Postgres) 连接。帐号密码分别是 homestead / secret

//附注： 因为虚拟机做了端口转发，所以本机电脑上你应当只使用这些非标准的连接端口，虚拟机里依然使用默认的 3306 及 5432 连接端口。


增加更多网站#
//一旦 Homestead 环境配置完毕且成功运行后，你可能会想要为 Laravel 应用程序增加更多的 Nginx 网站
//可以在单个 Homestead 环境中运行多个 Laravel 程序。在 Homestead.yaml 文件中增加另一个网站的设置后，进入 Homestead 目录并运行 vagrant provision 命令，即可新增一个网站

设置 Cron 调度器#
//通过 schedule:run Artisan 命令，调度便会在每分钟被运行。schedule:run 命令会检查定义在你 App\Console\Kernel 类中调度的任务，判断哪个任务该被运行

...
绑定#
//几乎所有的服务容器绑定都会注册至 服务提供者 中，所以下方所有的例子将示范在该情境中使用容器

//不过，如果它们没有依赖任何的接口，那么就没有将类绑定至容器中的必要。并不需要为容器指定如何建构这些对象，因为它会通过 PHP 的反射服务自动解析「实际」的对象

在服务提供者中，你总是可以通过 $this->app 实例变量访问容器
//我们可以使用 bind 方法注册一个绑定，传递我们希望注册的类或接口名称，并连同返回该类实例的闭包：
...
//这么做会告知容器当有个类需要 EventPusher 的实现时，必须注入 RedisEventPusher
//现在我们可以在构造器中对 EventPusher 接口使用类型提示，或任何需要通过服务容器注入依赖的其它位置：
...
//例如，当系统收到新订单时，我们可能想通过 PubNub 来发送事件，而不是 Pusher
...
//例如，你正在构建一个能接收多个不同 Report 接口实现数组的报表汇整器。注册完 Report 实现后，可以使用 tag 方法为它们赋予一个标签：

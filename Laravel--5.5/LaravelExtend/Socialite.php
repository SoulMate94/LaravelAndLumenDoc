...
官方文档#
除了常规的基于表单的身份验证之外， Laravel 也提供了一种简单，方便的办法来使用 Laravel 社会化登录 向 OAuth 提供程序进行身份验证

社公化登录目前支持 Facebook ， Twitter ， LinkedIn ，Google ，GitHub 和 Bitbucket 的身份验证。
...
基本用法#
你需要两个路由：一个重定向用户到 OAuth 提供商，另一个在提供商验证之后接收回调。我们用 Socialite facade 来访问：
<?php
...
redirect 方法负责发送用户到 OAuth 提供商，而 user 方法将读取传入的请求并从提供商处检索用户信息

在重定向用户之前，你还可以加入附加的 scope 方法来设置请求的「scope」。这个方法会覆盖所有现有的范围。

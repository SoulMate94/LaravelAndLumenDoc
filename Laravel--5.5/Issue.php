...
版本控制方案#
//Laravel 的版本控制方案继续以下约定： 主版本号.次版本号.修订号。次版本号的框架会在每六个月发布一次 (二月和八月)，修订号发布可能会每周发布一次。而修订号版本不应该包含破坏性更改。

为什么 Laravel 不使用语义版本控制？#
//原因是语义版本控制是确定两段代码是否兼容的「简化」方法。即使是使用语义版本控制，你仍然必须安装升级包并运行你的自动化测试组建，来确定事实上是否有任何异常与代码不兼容
...
API 资源#
//构建 API 时，你可能需要一个位于 Eloquent 模型和实际返回给用户响应之间的 JSON 转换层
//Laravel 的资源类允许你以轻松地方式将你的模型和模型集合转换为 JSON。而这个资源类代表了需要转换为 JSON 结构的单个模型
例如，这里是一个简单的用户资源类：
<?php
...
!!这个命令只能用在新创建的框架中，而不应该在现有的应用中使用它。

队列任务链#
//任务链允许你指定按顺序运行的队列任务列表。如果队列中的一个任务失败，则其余任务便不会再运行
//要执行队列的任务链，你可以在分配任务时使用 withChain 方法：
...
Queued 任务速率限制#
//如果你的应用程序与 Redis 进行交互，那你就可以根据时间或并发来调整排队的任务
//当你队列的任务与限制速率的 API 进行交互时，这个功能就派上用场了。例如，你可以限制给定类型的任务只能每 60 秒运行 10 次：
...
!!在上面的示例中，key 是唯一可以标识要限制的任务类型的字符串。 例如，你可能会根据任务的类名和其运行的 Eloquent 模型的 ID 来构建这个 key。
...
基于时间的任务尝试#
//作为在任务失败之前定义任务可能尝试多少次的替代方法，现在你可以定义任务的超时时间
//这允许在给定时间范围内尝试次数的任务。将 retryUntil 方法添加到任务类中来定义任务的超时时间：
/**
 * 确定任务的超时时间。
 *
 * @return \DateTime
...
!!你也可以在队列的事件监听器上定义一个 retryUntil 方法。
...
验证对象只有两个方法：passes 和 message
--passes 方法接收属性值和名称，并根据属性值是否有效返回 true 或 false
...
// 所有验证错误的 JSON 格式可以通过在 App\Exceptions\Handler 类中定义单个方法来控制
//例如，下面将使用 Laravel 5.4 方式进行来自定义验证响应的 JSON 格式：
...
缓存锁#
//Redis 和 Memcached 缓存驱动程序现在支持获取和释放原子「锁
//这提供了一种在不考虑竞争条件的情况下获取任意锁的简单方法
举个例子，在执行任务之前，你可能希望获得给定值的锁定，来确保没有其他进程在执行相同的任务：
...
Blade 改进#
//编写一个自定义指令有时候比定义简单的自定义条件语句更复杂。因此 Blade 现在提供一个 Blade::if 方法，它允许你使用闭包快速定义自定义条件指令
//举个例子，定义一个检查当前应用程序环境的自定义条件，我们可以在我们的 AppServiceProvider 的 boot 方法这样做：
se Illuminate\Support\Facades\Blade;
...
//同样的效果，如果你的路由只需要返回一个视图，使用 Route::view 方法
//view 方法接受一个 URI 作为第一个参数，视图名称作为其第二个参数。另外，你可以提供一个数组作为可选的第三个参数传递给视图：
...
//sticky 选项是一个可选的值，可用于在当前请求周期内立即读取已写入数据库的记录。如果启用了 sticky 选项，并且在当前请求周期内对数据库执行了「写入」操作，任何进一步的「读取」操作将使用「写入」连接
//这确保了在该请求周期期间写入的任何数据可以在同一请求期间立即从数据库读回。你可以决定这是否是你应用程序所需的行为

...
介绍#
//但是如何满足 API 场景下的授权需求呢？在 API 场景里通常通过令牌来实现用户授权，而非维护请求之间的 Session 状态

//现在 Laravel 项目中可以使用 Passport 轻而易举地实现 API 授权过程，通过 Passport 可以在几分钟之内为你的应用程序添加完整的 OAuth2 服务端实现。 Passport 基于 League OAuth2 server 该项目的维护人是 Alex Bilbie。
OAuth2


安装#
//使用 Composer 依赖包管理器安装 Passport ：
composer require laravel/passprot
...
//面命令执行后，请将 Laravel\Passport\HasApiTokens Trait 添加到 App\User 模型中，这个 Trait 会给你的模型提供一些辅助函数，用于检查已认证用户的令牌和使用作用域：
...
//最后，需要将配置文件 config/auth.php 中 api 部分的授权保护项（ driver ）改为 passport
...
//这些组件注册后，请保证运行 npm run dev 命令重新编译你的代码

//一旦你重新编译了代码，你可以直接将这些组件直接放入应用程序的模板中，用于创建客户端和私人访问令牌：
...
配置#
令牌的有效期#
//默认情况下，Passport 发放的访问令牌是永久有效的，不需要刷新。但是如果你想给访问令牌配置一个短一些的有效期，那你就需要用到 tokensExpireIn 和 refreshTokensExpireIn 方法了
...
命令 passport:client#
//创建客户端最简单的方式是使用 Artisan 命令 passport:client ，你可以使用此命令创建自己的客户端，用于测试 OAuth2 的功能
//在你执行 client 命令时，Passport 会提示输入更多关于你的客户端的信息，最终会提供给你生成的客户端的 ID 和 密钥：
...
请求令牌#
授权时的重定向#
//客户端创建之后，开发者会使用此客户端的 ID 和密钥向你的应用程序请求一个授权码和访问令牌
//首先，接入应用会将用户重定向到你应用程序的 /oauth/authorize 路由上，示例如下：
...
!!注意，路由 /oauth/authorize 已经在 Passport::routes 方法中定
...
将授权码转换为访问令牌#
//用户允许授权请求后，用户将会被重定向会接入应用程序，然后接入应用将通过 POST 请求向你的应用程序申请访问令牌，此次请求需要携带用户允许授权时产生的授权码。
在下面的例子中，我们使用 Guzzle HTTP 库来实现这次 POST 请求：
...
//如上 /oauth/authorize 路由，/oauth/token 已经在 Passport::routes 方法中定义，所以无需再次定义。
...
请求密码授权令牌#
//当你创建密码授权客户端后，你可以向 /oauth/token 接口发起 POST 请求来获取访问令牌，请求时需要带有用户的邮箱地址和密码信息
注意，该接口已经在 Passport::routes 方法中定义，所以无需再次手动定义
//请求成功后，服务端返回的 JSON 响应数据中会带有 access_token 和 refresh_token 属性
$http = new GuzzleHttp\Client;
...
请求所有作用域#
//使用密码授权机制时，你可以通过请求作用域 * 让你的令牌获取应用程序中定义的所有作用域
//在处理使用此令牌发起的请求时，can 函数会始终返回 true ，这种作用域的授权最好只应用在使用 password 授权时发放的令牌中：
$response = $http->post('http://your-app.com/oauth/token', [
    'form_params' => [
        'grant_type' => 'password',
        'client_id' => 'client-id',
...
简化授权令牌#
//简化授权和通过授权码授权相似; 区别是, 不需要通过授权码去获取令牌而是把令牌直接返回客户端. 主要用在无法安全存储证书场景中
//这种授权在 JavaScript 和 移动应用 是最常用的. 开启授权, 在 AuthServiceProvider 中调用 enableImplicitGrant 方法:
/**
 * Register any authentication / authorization services.
 *
 * @return void
...
!!记住, 这个 /oauth/authorize 接口已经定义在 Passport::routes 中，所以无需再次手动定义。


客户端授权令牌#
客户端证书授权适用于机器对机器认证
//，例如，你可以在通过API执行脚本任务中使用此授权。要使用这种授权，你首先需要在 app/Http/Kernel.php 的 $routeMiddleware 变量中添加新的中间件：
...
JSON API#
//Passport 中也有用来管理私人访问令牌的 JSON API，你可以基于这些 API 开发一套前端操作界面供给用户管理自己的私人访问令牌
//下面我们会列出所有管理私人访问令牌的 API。方便起见，我们使用 Axios 展示对 API 的 HTTP 请求。
...
路由保护#
通过中间件#
//面i将 api 中的保护机制改为为 passport 后，你只要给需要验证访问令牌的路由添加 auth:api 中间件，该机制将发挥作用
Route::get('/user', function () {
    //
})->middleware('auth:api');

传递访问令牌#
//接入应用在调用 Passport 保护下的路由时，需要将访问令牌作为 Bearer 令牌放在请求头 Authorization 中
...
令牌作用域#
定义作用域#
//当 API 客户端接入特定用户时，可以通过作用域来限定其访问权限
你可以使用 Passport::tokensCan 方法来定义 API 的作用域，定义代码需要放置在 AuthServiceProvider 的 boot 方法中
//tokensCan 方法接受一个包含作用域名称、描述的数组作为参数。作用域描述将会在授权确认页中直接展示给用户，你可以将其定义为任何你需要的内容：
...
给令牌分派作用域#
授权码机制#
//使用授权码机制申请访问令牌时，接入应用可以通过 scope 字符串参数指定他们需要的作用域
//scope 包含多个作用域名称时，名称之间使用空格分隔：
Route::get('/redirect', function () {
    $query = http_build_query([
        'client_id' => 'client-id',
        'redirect_uri' => 'http://example.com/callback',
...
检查作用域#
//Passport 包含两个检查作用域的中间件，通过访问令牌请求时将会使用这两个中间件来检查是否授予了特定作用域
要将下面的中间件添加到 app/Http/Kernel.php 文件的 $routeMiddleware 属性中：
'scopes' => \Laravel\Passport\Http\Middleware\CheckScopes::class,
'scope' => \Laravel\Passport\Http\Middleware\CheckForAnyScope::class,

检查所有作用域#
//由可以使用 scopes 中间件来检查当前请求是否拥有指定的 所有 作用域：
...
// 在构建 API 时，如果能通过 JavaScript 应用接入自己的 API 将会给开发过程带来极大的便利
//同样的 API 可以被你自己的 web 应用、移动应用、第三方应用以及你发布到各个包管理平台的 SDK 共同使用。

//你需要做的仅仅是将 CreateFreshApiToken 中间件添加到你的 web 中间件组中：
'web' => [
    // Other middleware...
...
!!如果你用了其他 JavaScript 框架，需要确保每次对外请求都会带有 X-CSRF-TOKEN 和 X-Requested-With 请求头
...
测试#
//Passport 的 actingAs 方法可以用于指定当前认证的用户及其授权范围。 
//actingAs 方法第一个参数是一个对象，第二个参数是数组表示申请的授权范围:

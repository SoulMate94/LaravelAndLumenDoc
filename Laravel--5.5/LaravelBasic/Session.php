...
默认情况下，Laravel 配置了适用于大多数应用程序的 file Session 驱动
在生产环境下，你可以考虑使用 memcached 或 redis 驱动来实现更出色的 Session 性能
...
首先，我们来看看通过控制器方法类型提示一个 Request 实例来访问 session
<?php
...
当你从 Session 获取值时，你还可以传递一个默认值作为 get 方法的第二个参数。如果 Session 中不存在指定的键，便会返回这个默认值
...
使用单个字符串类型的值作为参数调用辅助函数 session 时，它会返回字该符串对应的 Session 键的值
当使用一个键值对数组作为参数调用辅助函数 session 时，传入的键值将会存储在 Session 中：
...
!!通过 HTTP 请求实例操作 Session 与使用全局辅助函数 session 两者之间并没有实质上的区别。这两种方法都可以通过所有测试用例中可用的 assertSessionHas 方法进行 测试
...
例如，假设 user.teams 这个键是包含团队名称的数组，你可以像这样将一个新的值加入到此数组中：
...
闪存数据#
有时候你仅想在下一个请求之前在 Session 中存入数据，你可以使用 flash 方法
使用这个方法保存在 session 中的数据，只会保留到下个 HTTP 请求到来之前，然后就会被删除。闪存数据主要用于短期的状态消息：
$request->session()->flash('status', 'Task was successful!');

如果需要保留闪存数据给更多请求，可以使用 reflash 方法，这将会将所有的闪存数据保留给其他请求
如果只想保留特定的闪存数据，则可以使用 keep 方法：
...
添加自定义 Session 驱动#
实现驱动#
你自定义的 Session 驱动必须实现 SessionHandlerInterface 接口
这个接口包含了一些我们需要实现的简单方法。下面是一个大概的 MongoDB 实现流程示例
<?php
...
！！Laravel 默认没有附带扩展用的目录，你可以把它放在你喜欢的目录内。在下面这个例子中，我们创建了一个 Extensions 目录放置自定义的 MongoHandler 扩展。
...
注册驱动#
你的 Session 驱动实现之后，你还需要在框架中注册该驱动，即将该扩展驱动添加到 Laravel Session 后台
然后在 服务提供者 的 boot 方法内调用 Session Facade 的 extend 方法
之后你就可以从现有的 AppServiceProvider 或者新创建的提供器中执行此操作。
<?php

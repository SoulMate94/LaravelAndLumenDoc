...
通过修改 config/app.php 配置文件中的 log 选项来配置 Laravel 使用的存储机制
如果你希望每天产生日志都存放在不同的文件中，则应将 app 配置文件中的 log 值设置为 daily：
'log' => 'daily'
...
日志严重程度级别#
生产环境中通过将 log_level 选项添加到 app.php 配置文件中来配置应当记录的严重程度最低的日志级别
...
自定义渠道名称#
默认情况下，Monolog 用与当前环境匹配的名称进行实例化，如 production 或 local。
要更改此值，可将 log_channel 选项添加到 app.php 配置文件中：
...
report 方法用于记录异常或将其发送到外部服务，如 Bugsnag 或 Sentry。默认情况下，report 方法只是简单地将异常传递给记录异常的基类。你可以根据需要来记录异常。
...
按类型忽略异常#
异常处理程序的 $dontReport 属性包含不会被记录的异常类型数组
例如，404错误导致的异常以及其他类型的错误不会写入日志文件。你可以根据需要向此数组添加其他异常类型
/**
 * 不应报告的异常类型列表。
 *
 * @var array
...
Render 方法#
render 方法负责将给定的异常转换成发送给浏览器的 HTTP 响应
默认情况下，异常会传递为你生成响应的基类。你还可以根据需要检查异常类型或返回自定义的响应：
/**
 * 渲染异常到 HTTP 响应中.
 *
...
你并不一定要在异常处理程序中的 report 和 render 方法中处理不同类型的异常，可以直接在自定义的异常处理程序中定义 report 和 render 方法
...
HTTP 异常#
一些异常描述了来自服务器的 HTTP 错误代码
例如，可能是错误代码 404 的「找不到页面」、401 的「未授权错误」甚至可能是由开发者造成的 500
你可以使用辅助函数 abort 在应用程序中的任何地方生成这样的响应：
abort(404);
...
自定义 HTTP 错误页面#
Laravel 可以轻松地显示各种 HTTP 状态代码的自定义错误页面。
例如，如果你要自定义 404 HTTP 状态代码的错误页面，就创建一个 resources/views/errors/404.blade.php
此文件将会用于渲染你应用中产生的所有 404 错误

此目录中的视图文件的命名应该与它们对应的 HTTP 状态代码匹配。由 abort 函数引发的 HttpException 实例将作为 $exception 变量传递给视图。
<h2>{{ $exception->getMessage() }}</h2>

日志#
Laravel 在强大的 Monolog 库上提供了一个简单的抽象层。默认情况下，Laravel 的日志文件的存储目录被配置为 storage/logs 。
你可以使用 Log facade 将信息写入日志：
<?php
...
该日志记录器提供 RFC 5424 中定义的八种日志级别：emergency、alert、critical、error、warning、notice、info 和 debug

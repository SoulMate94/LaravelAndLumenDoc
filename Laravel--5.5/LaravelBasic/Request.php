...
HTTP 请求#
获取请求#
要通过依赖注入的方式来获取当前 HTTP 请求的实例，你应该在控制器方法中类型提示 Illuminate\Http\Request。传入的请求的实例将通过 服务容器 自动注入：
<?php
...
获取请求路径#
path 方法返回请求的路径信息
也就是说，如果传入的请求的目标地址是 http://domain.com/foo/bar，那么 path 将会返回 foo/bar：
$uri = $request->path();
...
// 那么你首先要安装几个函数库。Laravel 使用 Symfony 的 HTTP 消息桥接组件将典型的 Laravel 请求和响应转换为 PSR-7 兼容实现：
...
!!如果从路由或者控制器返回 PSR-7 响应实例，它会自动转换回 Laravel 响应实例，并由框架显示。
...
通过动态属性获取输入#
你也可以通过 Illuminate\Http\Request 实例的动态属性来获取用户输入。
// 例如，如果你应用的表单中包含 name 字段，那么可以像这样访问该字段的值：
$name = $request->name;
...
若要从请求中获取 cookie 值，你可以在 Illuminate\Http\Request 实例上使用 cookie 方法：
$value = $request->cookie('name');


将 Cookies 附加到响应#
你可以使用 cookie 方法将 cookie 附加到传出的 Illuminate\Http\Response 实例
你需要传递 Cookie 名称、值、以及有效期（分钟）到这个方法：

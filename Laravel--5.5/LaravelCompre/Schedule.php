...
该 Cron 将于每分钟调用一次 Laravel 命令调度器，当 schedule:run 命令执行时， Laravel 会评估你的计划任务并运行预定任
...
在开始之前，先让我们来看看一个任务的调度示例。在该例子中，我们计划了一个会在每天午夜被调用的 Closure 。
该 Closure 将运行数据库查询语句来清除某个数据表：
<?php
...
除了计划 Closure 调用，你还能计划 Artisan 命令 以及系统命令操作
举个例子，你可以使用 command 方法传参命令名称或者命令类名称来计划一个 Artisan 命令：
...
如果指定的 Closure 返回 true，且没有其它限制条件存在，那么这个任务将会被继续运行。
...
这是因为我们考虑到你可能在维护模式下做一些破坏性的维护工作，我们不想让任务调度对这些工作造成干扰
然而，如果你想强制某个任务在维护模式下运行的话，你可以使用 evenInMaintenanceMode 方法：
...
注意，你必须先通过 sendOutputTo 方法将其输出到一个文件。同时，在邮件发出之前，你需要先设置 Laravel 的 电子邮件服务:
$schedule->command('foo')
         ->daily()
         ->sendOutputTo($filePath)
         ->emailOutputTo('foo@example.com');
!!emailOutputTo， sendOutputTo 和 appendOutputTo 方法只适用于 command 方法，不支持 call 方法。
...
该方法在你计划的任务进行或完成时，可用来有效的通知一个外部服务，例如 Laravel Envoyer：
$schedule->command('emails:send')
         ->daily()
         ->pingBefore($url)
         ->thenPing($url);

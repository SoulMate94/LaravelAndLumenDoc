...
根目录#
	...
	storage 目录
		//storage 目录包含编译的 Blade 模板、基于文件的会话和文件缓存、以及框架生成的其他文件。这个目录被细分成 app、framework 和 logs 三个子目录
...
App 目录#
//应用程序的核心代码位于 app 目录内。默认情况下，这个目录位于命名空间 App 下，并由 Composer 按照 PSR-4 自动载入标准 来自动加载。
//当你通过 Artisan 提供的 make 命令来生成类时，会在 app 中生成各种各样的目录。例如，在执行 Artisan 命令 make:job 来生成任务类之前，app/Jobs 目录都不会存在
	...
	Jobs 目录
		//Jobs 目录默认是不存在的，它会在你运行Artisan 命令 make:job 时生成。
		//这个目录存放了应用中的 队列任务。应用的任务可以被推送到队列或者在当前请求的生命周期内同步运行。在当前请求期间同步运行的任务可以看做是一个「命令」，因为它们是 命令模式 的实现。
	Listeners 目录
		//Listeners 目录默认是不存在的，它会在你运行 Artisan 命令 event:generate 或 make:listenr 时生成。
		//Listeners 目录包含了用来处理 事件 的类
		//事件监听器接收事件实例并执行响应该事件被触发的逻辑。例如，UserRegistered 事件可能由 SendWelcomeEmail 监听器处理。
	Mail 目录
		//Mail 目录默认不存在，它会在你运行 Artisan 命令 make:mail 时生成。
		//Mail 目录包含应用所有的邮件发送类。邮件对象允许你将构建邮件的逻辑封装在可以使用 Mail::send 方法来发送邮件的地方
	Notifications 目录
		//Notifications 目录默认不存在，它会在你运行 Artisan 命令 make:notification 时生成。
		//Notifications 目录包含应用发送的所有「事务性」通知
		//比如关于在应用中发生的事件的简单通知。Laravel 的通知功能抽象了通知发送，可以通过各种驱动（例如邮件、Slack、短信）发送通知，或是存储在数据库中
	Policies 目录
		//Policies 目录默认不存在，它会通过运行 Artisan 命令 make:policy 来创建。
		//Policies 目录包含了应用的授权策略类。策略可以用来决定一个用户是否有权限去操作指定资源。更多详情可以查看 授权文档
	Providers 目录
	...
	Rules 目录
		//Rules 目录默认不存在，它会在运行 Artisan 命令 make:rule 命令时被创建。
		//Rules目录包含应用自定义验证规则对象。这些规则意在将复杂的验证逻辑封装在一个简单的对象中。更多详情可以查看 验证文档。
...
Valet#
简介#
//Valet 是 Mac 极简主义者的 Laravel 开发环境。没有 Vagrant，没有 /etc/hosts 文件。甚至可以使用本地隧道公开分享你的站点。 Yeah, we like it too.
//Laravel Valet 为你的 Mac 设置了启动后始终在后台运行 Nginx。然后，Valet 使用 DnsMasq 将所有指向安装在本地计算机的站点的请求代理到 *.dev 域上。
...
Laravel 还提供了另一种本地开发环境 Homestead。Homestead 和 Valet 的不同之处在于目标受众和对本地开发的方式。Homestead 提供了一个完整的、具有自动化的 Nginx 配置的 Ubuntu 虚拟机。如果你想要完全虚拟化的 Linux 开发环境或 Windows／Linux，Homestead 是一个不错的选择
...
安装#
//Valet 需要 macOS 和 Homebrew。在安装之前，要确保没有其他程序（如 Apache 或 Nginx ）占用了本地机器的端口 80。
...
//运行 valet links 命令可以查看所有目录链接的列表。你还可以使用 valet unlink app-name 来删除符号链
!!你可以使用 valet link 将多个（子）域指向同一个应用。要添加子域名或其它域名到应用，可以在应用目录下运行 valet link subdomain.app-name。
valet link subdomain.app-name


使用 TLS 构建安全站点#
默认情况下，Valet 使用 HTTP 协议提供站点
//但是，如果你想使用 HTTP/2 提供加密的 TLS 站点，使用 secure 命令。例如，如果你的站点域名是 laravel.dev，可以这样：
...
自定义 Valet 驱动#
//你可以编写自己的 Valet 「驱动」来提供运行在另一个不是由 Valet 支持的框架或 CMS 上的 PHP 应用程序

//安装 Valet 时，会创建一个包含 SampleValetDriver.php 文件 ~/.valet/Drivers 目录。这个文件包含实现驱动程序的例子，演示了如何编写自定义驱动程序。编写驱动程序只需要实现三种方法：serves、 isStaticFile 和 frontControllerPath。
$sitePath 是计算机上提供的站点的完整路径，例如 /Users/Lisa/Sites/my-project。
$siteName 是 「主机」 / 「站点名」 的域名部分，如 my-project。 
...
serves 方法#
如果你的驱动程序应该处理传入的请求，那么 serves 方法应该返回 true。否则，该方法应返回 false
//因此，在此方法中，你需要确定给定的 $sitePath 是否包含你要提供的类型的项目。
...
本地驱动#
//如果要为单个应用定义一个自定义 Valet 驱动程序，请在应用程序的根目录中创建一个 LocalValetDriver.php
//你的自定义驱动程序可以继承基础 ValetDriver 类或继承现有应用的特定驱动程序，如 LaravelValetDriver：
